...
public sealed class HttpSink : ISink
{
    ...
    {
        var resolvedUrl = string.IsNullOrWhiteSpace(url) ? Environment.GetEnvironmentVariable("OPENAKTA_SINK_URL") : url;
        if (string.IsNullOrWhiteSpace(resolvedUrl))
        {
            throw new ArgumentException("HttpSink requires a url or OPENAKTA_SINK_URL");
        }
        ...
        var token = Environment.GetEnvironmentVariable("OPENAKTA_SINK_TOKEN");
    ...
    public async Task ExportAsync(WideEventPayload @event)
    {
        ...
        {
            throw new HttpRequestException($"HTTP {(int)response.StatusCode} when exporting OPENAKTA Wide Event");
        }
    }
}

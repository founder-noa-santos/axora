...
public sealed class LoggerBuilder
{
    ...
    {
        var resolved = !string.IsNullOrWhiteSpace(candidate) ? candidate : System.Environment.GetEnvironmentVariable("AXORA_SERVICE");
        if (string.IsNullOrWhiteSpace(resolved))
        {
            throw new InvalidOperationException("Logger requires a service name or AXORA_SERVICE");
        }
    ...
    {
        var resolved = !string.IsNullOrWhiteSpace(candidate) ? candidate : System.Environment.GetEnvironmentVariable("AXORA_ENV");

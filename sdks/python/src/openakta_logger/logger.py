...
def _resolve_service(service: str | None) -> str:
    resolved = (service or os.getenv("OPENAKTA_SERVICE") or "").strip()
    if not resolved:
        raise ValueError("Logger requires a service name or OPENAKTA_SERVICE")
    return resolved


def _resolve_environment(environment: str | None) -> str:
    resolved = (environment or os.getenv("OPENAKTA_ENV") or "production").strip()

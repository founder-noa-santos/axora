...
class HttpSink(Sink):
    def __init__(
        ...
        *,
        ...
        resolved_url = url or os.getenv("AXORA_SINK_URL")
        if not resolved_url:
            raise ValueError("HttpSink requires a url or AXORA_SINK_URL")

        resolved_token = token or os.getenv("AXORA_SINK_TOKEN")

...
public class HttpSink implements Sink {
  ...
  public HttpSink(String url, Map<String, String> headers, int timeoutMs, HttpClient httpClient) {
    String resolvedUrl = url != null && !url.isBlank() ? url : System.getenv("AXORA_SINK_URL");
    if (resolvedUrl == null || resolvedUrl.isBlank()) {
      throw new IllegalArgumentException("HttpSink requires a url or AXORA_SINK_URL");
    }
    ...
    String token = System.getenv("AXORA_SINK_TOKEN");
              ...
              response -> {
                if (response.statusCode() >= 400) {
                  throw new CompletionException(
                      new IOException(
                          "HTTP " + response.statusCode() + " when exporting AXORA Wide Event"));
                }
                return null;
              });

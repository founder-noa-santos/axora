//! AXORA Backend Integration Tests
//!
...
use axora_core::{CollectiveServer, CoreConfig};
use axora_proto::collective::v1::{
    collective_service_client::CollectiveServiceClient,
    ListAgentsRequest, SubmitTaskRequest, Task,
};
...
async fn test_submit_task_grpc() {
    ...
    // Verify response
    assert!(response.into_inner().task.is_some(), "Task should be created");
...
async fn test_list_agents_grpc() {
    ...
    assert!(agents.len() >= 0, "Should return agent list");
...
async fn test_config_from_toml() {
    ...
    let config_content = r#"
        ...
        port = 50054
        database_path = "/tmp/test_axora.db"
        max_concurrent_agents = 5
        frame_duration_ms = 32
        debug = true
    "#;
    
    let config_path = "/tmp/test_axora_config.toml";
...
async fn test_frame_executor() {
    ...
    use axora_core::{Frame, FrameContext, FrameExecutor};
    use std::sync::Arc;

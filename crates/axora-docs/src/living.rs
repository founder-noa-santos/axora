...
impl LivingDocs {
    ...
    pub fn is_doc_stale(&self, doc_id: &str) -> bool {
        if let Some(doc) = self.docs.get(doc_id) {
            ...
            for (path, _) in &self.file_to_docs {
    ...
    fn analyze_change(
        ...
        let update_type = if diff_stats.lines_added + diff_stats.lines_removed > 50 {
            UpdateType::FlagForReview
        } else if diff_stats.lines_added + diff_stats.lines_removed > 10 {
            UpdateType::FlagForReview
        } else {
...
"#;
    ...
    fn test_living_docs_staleness() {
        ...
        doc.updated_at = doc.updated_at - 86400; // 1 day ago
        living_docs.add_document(doc).expect("Failed to add doc");
    ...
    fn test_living_docs_find_stale_docs() {
        ...
        doc2.updated_at = doc2.updated_at - 86400; // 1 day ago

...
pub struct ValidationResult {
    ...
    /// Structured diagnostic payload for rejected outputs.
    ...
    pub fn validate(&self, output: &str) -> Result<ValidatedAgentOutput> {
        ...
        if Self::looks_like_unified_diff(trimmed) {
            ...
            if parsed
                ...
                .iter()
                .flat_map(|h| &h.lines)
                .any(|line| matches!(line, openakta_cache::DiffLine::Context(_)))
            {
        ...
        Err(
            AgentError::DiffRequired("output does not match an accepted patch format".to_string())
                .into(),
        )
    }
        ...
        for target in diff.target_files() {
            ...
            if !patched.success {
                return Ok(PatchReceipt {
                    ...
                    message: patched
                        .error
                        .unwrap_or_else(|| "patch conflict".to_string()),
                    affected_files,
                });
            }

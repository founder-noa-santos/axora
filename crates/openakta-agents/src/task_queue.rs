...
//!
//! ```rust
//! use openakta_agents::{Task, TaskQueue, TaskQueueConfig};
//!
                ...
                if let Some(mut task) = self.tasks.get_mut(&task_id) {
                    match &task.status {
                        TaskQueueStatus::Pending | TaskQueueStatus::Blocked { .. } => {
                            ...
                            scheduler.add_task(task_id.clone(), task.priority).map_err(
                                |error| match error {
...
mod tests {
    ...
    fn add_task_rejects_missing_dependency() {
        ...
        assert_eq!(
            error,
            TaskQueueError::DependencyMissing("missing".to_string())
        );
    }
    ...
    fn dependency_resolution_unblocks_child_after_completion() {
        ...
        let child = queue
            .add_task(task("child"), 90, vec![parent.clone()])
            .unwrap();
    ...
    fn load_balancing_distributes_ready_tasks_evenly() {
        ...
        assert_eq!(
            assignments
                .iter()
                .map(|assignment| assignment.task_ids.len())
                .sum::<usize>(),
            4
        );

...
pub enum AgentError {
    ...
    /// Cloud execution path is unavailable.
    #[error("cloud execution unavailable: {0}")]
    CloudExecutionUnavailable(String),

    /// Cloud execution path is required.
    ...
    /// Destructive tool blocked while a HITL question is pending.
    #[error("BLOCKED_PENDING_ANSWER: {0}")]
    BlockedPendingAnswer(String),
}

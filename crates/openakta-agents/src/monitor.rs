            ...
            if let TaskStatus::InProgress { .. } = &task.status {
                ...
                if time_since_update > self.stall_threshold {
                    blockers.push(BlockerInfo {
                        task_id: *task_id,
                        reason: format!("Task stalled for {:?}", time_since_update.as_secs() / 60),
...
impl ProgressMonitor {
    ...
    pub fn new(blackboard: Arc<BlackboardV2>, mission_id: String) -> Self {
        ...
        Self {
            ...
            _detector: detector.clone(),
            reporter: Mutex::new(Some(StatusReporter::new(
                tracker,
                detector,
                mission_id.clone(),
            ))),
...
mod tests {
    ...
    fn test_task_status_progress_percentage() {
        ...
        assert_eq!(
            TaskStatus::Blocked(String::new()).progress_percentage(),
            0.0
        );
    }
    ...
    fn test_blocker_detector_detect_blocked_status() {
        ...
        let mut task = TaskProgress::new(1);
        task.update_status(TaskStatus::Blocked("Waiting for dependency".to_string()));
        tasks.insert(1, task);

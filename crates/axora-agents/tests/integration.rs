...
fn test_task_decomposition_simulation() {
    ...
    for (i, group) in parallel_groups.iter().enumerate() {
        println!("  Group {}: {:?}", i + 1, group.iter().map(|idx| subtasks[*idx]).collect::<Vec<_>>());
    }
}
...
fn test_agent_communication_protocol() {
    ...
    let message_types: Vec<&str> = message_queue.iter().map(|m| m.message_type.as_str()).collect();
    assert_eq!(message_types, vec!["task_assign", "review_request", "feedback", "task_complete"]);
    
    // Verify conversation completed
    let completion_message = message_queue.iter().find(|m| m.message_type == "task_complete");
    assert!(completion_message.is_some());
    
    println!("\n=== Agent Communication ===");
    for msg in &message_queue {
        println!("  {} -> {} [{}]: {}", msg.from, msg.to, msg.message_type, msg.payload);
    }
}
...
fn test_agent_state_management() {
    ...
    assert_eq!(agent.state, AgentState::Working("Implement login".to_string()));
    
    agent.wait_for("Review approval");
    assert_eq!(agent.state, AgentState::Waiting("Review approval".to_string()));
    ...
    assert!(agent.task_history.iter().any(|h| h.contains("Assigned: Implement login")));
    assert!(agent.task_history.iter().any(|h| h.contains("Completed: Fix bugs")));
...
fn test_multi_agent_coordination() {
            ...
            self.tasks.values()
                .filter(|t| {
                    ...
                    t.dependencies.iter().all(|dep| {
                        self.tasks.get(dep).map_or(false, |d| d.status == "completed")
                    })
                })
                .collect()
        }
    ...
    let assigned_count = coordinator.tasks.values()
...
fn test_agent_documentation_integration() {
    use axora_docs::{LivingDocs, Document, DocSchema, Adr, AdrLog};
    use std::path::Path;

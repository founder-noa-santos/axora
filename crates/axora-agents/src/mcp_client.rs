...
use axora_proto::mcp::v1::tool_service_client::ToolServiceClient;
use axora_proto::mcp::v1::{CapabilityPolicy, ListToolsRequest, ToolCallRequest};
...
impl McpClient {
    ...
    pub async fn call_tool(
        ...
        let mut client = self.connect().await?;
        let policy = policy.or_else(|| Some(Self::default_policy(agent_id, role, tool_name, workspace_root)));
        let result = client
        ...
        if result.success {
            ...
            Ok(Observation::failure(&result.stderr))
        }
    }

...
use crate::final_stage::{FusedCandidate, RetrievalDocument, UnifiedFinalStage, UnifiedFinalStageResult};
use crate::error::RagError;
use crate::{CrossEncoderScorer, Result};
use axora_embeddings::CodeEmbedder;
use axora_indexing::DenseVectorCollection;
    ...
    R: CrossEncoderScorer,
{
    ...
    pub async fn retrieve(&self, query: &CodeRetrievalQuery) -> Result<CodeRetrievalResult> {
        ...
        let fused_candidates = hits
            ...
            .filter(|candidate| matches_filters(candidate, &query.focal_files, &query.focal_symbols))
            .collect::<Vec<_>>();
...
fn hydrate_candidate(
    workspace_root: &Path,
    hit: axora_indexing::DenseSearchResult,
    rank: usize,
    ...
    let start_line = hit
        ...
        .and_then(as_usize)
        .unwrap_or(1);
    let end_line = hit.payload.get("end_line").and_then(as_usize).unwrap_or(start_line);

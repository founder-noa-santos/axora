...
impl Default for CrossEncoderConfig {
    fn default() -> Self {
        let model_root = std::env::var("AXORA_CROSS_ENCODER_MODEL_ROOT")
            .map(PathBuf::from)
            .unwrap_or_else(|_| PathBuf::from(".axora/models/cross-encoder"));
        Self {
            model_root,
            ...
            max_length: 256,
            batch_size: 8,
            device: std::env::var("AXORA_CROSS_ENCODER_DEVICE").unwrap_or_else(|_| "cpu".to_string()),
        }
    }
}
            ...
            let encodings = batch
                ...
                .map(|doc| {
                    self.runtime
                        ...
                        .map(|mut encoding| {
                            encoding.truncate(self.config.max_length, 0, tokenizers::TruncationDirection::Right);
                            encoding
                        })
                })
                .collect::<std::result::Result<Vec<_>, _>>()
                .map_err(AxoraRagError::from)?;
...
impl CachedCrossEncoder {
    fn load(config: &CrossEncoderConfig) -> Result<Self> {
        ...
        let tokenizer = Tokenizer::from_file(tokenizer_path)
            .map_err(|err| RagError::Rerank(err.to_string()))?;
        let model = BertSequenceClassifier::load(&model_config_path, &weights_path, &config.device)?;
...
impl BertSequenceClassifier {
    fn load(config_path: &Path, weights_path: &Path, device_name: &str) -> Result<Self> {
        let config = std::fs::read_to_string(config_path)
            .map_err(|err| RagError::Rerank(err.to_string()))
            .and_then(|content| serde_json::from_str::<BertConfig>(&content).map_err(|err| RagError::Rerank(err.to_string())))?;

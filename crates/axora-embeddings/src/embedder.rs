...
impl EmbeddingModel for JinaCodeEmbedder {
    ...
    async fn embed(&self, input: &str) -> Result<Vec<f32>> {
        self.runtime.with_inference_lock(|| {
            ...
            Ok(generate_domain_embedding(input, self.profile.dimensions, 17))
        })
    }
}
...
impl EmbeddingModel for BgeSkillEmbedder {
    ...
    async fn embed(&self, input: &str) -> Result<Vec<f32>> {
        self.runtime.with_inference_lock(|| {
            ...
            Ok(generate_domain_embedding(input, self.profile.dimensions, 31))
        })
    }
}
...
mod tests {
    ...
    async fn skill_embedder_uses_expected_dimensions() {
        ...
        assert_eq!(embedder.embed("retrieve cargo repair instructions").await.unwrap().len(), 384);
    }

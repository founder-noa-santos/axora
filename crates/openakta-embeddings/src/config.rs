...
impl Default for CodeEmbeddingConfig {
    fn default() -> Self {
        let model_root = std::env::var("OPENAKTA_CODE_EMBED_MODEL_ROOT")
            .map(PathBuf::from)
            .unwrap_or_else(|_| PathBuf::from(".openakta/models/jina-code-embeddings-v2"));
        Self {
            ...
            max_length: 8192,
            batch_size: 16,
            device: std::env::var("OPENAKTA_CODE_EMBED_DEVICE")
...
impl Default for SkillEmbeddingConfig {
    fn default() -> Self {
        let model_root = std::env::var("OPENAKTA_SKILL_EMBED_MODEL_ROOT")
            .map(PathBuf::from)
            .unwrap_or_else(|_| PathBuf::from(".openakta/models/bge-small-en-v1.5"));
        Self {
            ...
            max_length: 512,
            batch_size: 32,
            device: std::env::var("OPENAKTA_SKILL_EMBED_DEVICE")
                .unwrap_or_else(|_| "cpu".to_string()),
        }
    }
}

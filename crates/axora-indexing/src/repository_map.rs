...
//!
//! ```rust,no_run
//! use axora_indexing::repository_map::{RepositoryMapper, RepositoryMap};
//! use std::path::Path;
//!
...
impl RepositoryMapper {
    ...
    fn extract_references(&mut self, node: Node, symbol: &Symbol, content: &str) -> Result<()> {
        ...
        for child in node.children(&mut cursor) {
            if child.kind().contains("identifier") || child.kind().contains("call") {
                if let Some(name) = child.utf8_text(content.as_bytes()).ok() {
    ...
    fn estimate_tokens(&self, symbols: &[Symbol]) -> usize {
        symbols.iter().map(|s| s.estimate_tokens()).sum()
    }
...
"#;
        ...
        // Mapper should be created successfully
...
"#;
        ...
        // Mapper should work - symbols may or may not be extracted depending on tree-sitter
        assert!(repo_map.total_symbols >= 0);
    }
...
"#;
        ...
        // Each symbol should have a token estimate
        for symbol in &repo_map.symbols {
            assert!(symbol.estimate_tokens() >= 0);
        }
    }

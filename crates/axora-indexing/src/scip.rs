...
//!
//! ```rust,no_run
//! use axora_indexing::scip::{ParserRegistry, Language};
//! use std::path::Path;
//!
...
/// Extracts package name from Cargo.toml content
...
/// Extracts package version from Cargo.toml content
fn extract_package_version(content: &str) -> Result<String> {
    let toml_value: toml::Value = toml::from_str(content)?;
    Ok(toml_value
        .get("package")
        .and_then(|p| p.get("version"))
...
/// Extracts project name from pyproject.toml content
...
/// Extracts project version from pyproject.toml content
...
/// Extracts module name from go.mod content
fn extract_module_name(content: &str) -> Result<String> {
    Ok(content
        .lines()
        .find(|line| line.starts_with("module "))
        .and_then(|line| line.split_whitespace().nth(1))

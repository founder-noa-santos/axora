...
use crate::Result;
use axora_embeddings::SkillEmbedder;
...
impl TantivySkillIndex {
    ...
    pub fn search(&self, query: &str, limit: usize) -> Result<Vec<SparseSkillHit>> {
        ...
        let combined = BooleanQuery::new(
            std::iter::once((Occur::Should, Box::new(parsed) as Box<dyn tantivy::query::Query>))
                .chain(terms)
                .collect(),
        );
        let docs = searcher
...
mod tests {
    ...
    use axora_embeddings::{BgeSkillEmbedder, SkillEmbeddingConfig};

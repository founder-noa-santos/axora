...
//!
//! ```rust,no_run
//! use openakta_cache::context_pruning::ContextManager;
//!
//! # fn main() -> Result<(), Box<dyn std::error::Error>> {
//! // Context manager requires an influence graph
//! // See openakta_indexing::influence::InfluenceGraph for details
//! # Ok(())
//! # }
//! ```
...
use openakta_indexing::influence::InfluenceGraph;
...
pub enum ContextPruningError {
    ...
    InfluenceGraph(#[from] openakta_indexing::influence::InfluenceError),
    ...
    pub fn from_file(path: &Path) -> Result<Self> {
        ...
        if let Some(after_open) = content.strip_prefix("---") {
            ...
            if let Some(close) = after_open.find("---") {
                ...
                body = after_open[close + 3..].trim().to_string();
...
impl ContextManager {
    ...
    fn lookup_symbol(&self, module: &str, symbol: &str) -> Option<String> {
        ...
        let _qualified_symbol = format!("{}::{}", module, symbol);
        ...
        for file in mentioned_files {
            ...
            if let Some(parent) = file.parent() {
                ...
                for entry in walkdir::WalkDir::new(parent).max_depth(3) {
                    let entry = entry?;
                    if entry.path().extension().is_some_and(is_code_extension) {
...
mod tests {
    ...
    fn test_mentioned_file_extraction() {
        ...
        let _files = pruner.extract_mentioned_files(&task).unwrap();
...
"#;
    ...
    fn test_context_pruning() {
        ...
        let _context = pruner.allocate(&task, &agent).unwrap();
    ...
    fn test_95_percent_savings() {
        ...
        let _task = Task::new("test-1", "Update src/file0.rs");
    ...
    fn test_deterministic_graph_traversal() {
        ...
        let _ = d_vector.transitive_closure.len();
    }

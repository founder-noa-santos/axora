...
[![Crates.io](https://img.shields.io/crates/v/openakta-cache.svg)](https://crates.io/crates/openakta-cache)
[![Documentation](https://docs.rs/openakta-cache/badge.svg)](https://docs.rs/openakta-cache)
[![Tests](https://github.com/openakta/aktacode/actions/workflows/rust-ci.yml/badge.svg)](https://github.com/openakta/aktacode/actions)

## Overview

OPENAKTA Cache is a high-performance caching and token optimization library designed for LLM-based development agents. It provides multiple strategies to reduce token consumption by **90%+** while maintaining sub-20ms latency.
...
```rust
use openakta_cache::{PrefixCache, CachedPromptBuilder};
...
```rust
use openakta_cache::{calculate_token_savings, UnifiedDiff};
...
```rust
use openakta_cache::{CompactorConfig, Context, ContextCompactor, ContextEntry, ItemKind};
...
```bash
cargo test -p openakta-cache toon
cargo test -p openakta-cache context
cargo test -p openakta-cache minifier
cargo test -p openakta-cache context_compactor
```

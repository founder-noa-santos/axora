...
use axora_embeddings::{BgeSkillEmbedder, SkillEmbedder, SkillEmbeddingConfig};
use axora_indexing::{
...
use axora_proto::mcp::v1::{
    CandidateScore, RetrievalDiagnostics, RetrieveSkillsRequest, RetrieveSkillsResponse, RetrievedSkill,
};
use axora_rag::{
    CandleCrossEncoder, CrossEncoderScorer, MemgasResult as SharedMemgasResult,
    RankedHit, RerankDocument, ReciprocalRankFusion, RetrievalDocument,
    SelectionResult as SharedSelectionResult, UnifiedFinalStage,
};
use chrono::Utc;
...
impl SkillDocument {
    ...
    fn as_rerank_document(&self) -> RerankDocument {
        RerankDocument {
...
impl SkillCatalog {
    ...
    pub fn delete_document(&self, skill_id: &str) -> Result<()> {
        let conn = Connection::open(&self.db_path)?;
        conn.execute("DELETE FROM skill_documents WHERE skill_id = ?1", [skill_id])?;
        conn.execute("DELETE FROM skill_source_state WHERE skill_id = ?1", [skill_id])?;
        Ok(())
    }
...
impl SkillCorpusIngestor {
    ...
    async fn parse_snapshot(&self, snapshot: &FastFileSnapshot) -> Result<(SkillDocument, SkillSourceState)> {
...
async fn build_skill_dense_collection(
    ...
    match config.dense_backend {
        VectorBackendKind::Qdrant => Ok(Arc::new(
            axora_indexing::QdrantVectorCollection::new(
                &config.qdrant_url,
        ...
        VectorBackendKind::SqliteVec => Ok(Arc::new(
            axora_indexing::SqliteVecCollection::new(
    ...
    pub async fn search(
        ...
        for rank in fused.into_iter().take(96) {
            ...
            if let Some(document) = catalog.get_document(&rank.document_id)? {
                let dense = dense_hits.iter().find(|hit| hit.skill_id == rank.document_id).cloned();
                let sparse = sparse_hits.iter().find(|hit| hit.skill_id == rank.document_id).cloned();
...
impl SkillRetrievalPipeline<HybridSkillIndex, CandleCrossEncoder> {
    ...
    pub async fn new(config: SkillRetrievalConfig) -> Result<Self> {
        ...
        let reranker = CandleCrossEncoder::new()
            .map_err(|err| ProceduralError::Retrieval(err.to_string()))?;
    ...
    R: CrossEncoderScorer,
{
    ...
    pub async fn retrieve(&self, request: &RetrieveSkillsRequest) -> Result<RetrieveSkillsResponse> {
        ...
        let shared_candidates = fused_candidates
            .iter()
            .cloned()
            .map(|candidate| axora_rag::FusedCandidate {
        ...
        Ok(RetrieveSkillsResponse {
            ...
            diagnostics: if request.include_diagnostics {
                Some(RetrievalDiagnostics {
                    dense_hits: fused_candidates.iter().filter(|item| item.dense_rank.is_some()).count() as u32,
                    bm25_hits: fused_candidates.iter().filter(|item| item.bm25_rank.is_some()).count() as u32,
...
fn build_candidate_scores(
    ...
    let accepted_scores = memgas
        .accept_set
        .iter()
        .map(|item| (item.candidate.document.skill_id.clone(), item.accept_posterior))
...
fn split_frontmatter(content: &str) -> Result<(serde_json::Value, String)> {
    if !content.starts_with("---\n") {
        return Ok((serde_json::Value::Object(Default::default()), content.to_string()));
    }
    ...
    let value = serde_yaml::from_str::<serde_yaml::Value>(yaml)
        .map_err(|err| ProceduralError::Serialization(err.to_string()))?;
    let json = serde_json::to_value(value).map_err(|err| ProceduralError::Serialization(err.to_string()))?;
...
fn standardize_scores(candidates: &[FusedCandidate]) -> Vec<f32> {
    let scores = candidates.iter().map(|candidate| candidate.rrf_score).collect::<Vec<_>>();
    ...
    scores.into_iter().map(|score| (score - mean) / stddev).collect()
}
...
mod tests {
    use super::*;
    use axora_rag::CrossEncoderScorer;
    use std::sync::Arc;

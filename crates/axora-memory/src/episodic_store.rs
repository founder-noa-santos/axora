...
//!
//! This module implements episodic memory storage for AXORA agents:
...
//!
//! ```rust,no_run
//! use axora_memory::{EpisodicStore, EpisodicMemory, MemoryType};
//! use chrono::Utc;
//!
//! #[tokio::main]
...
impl MemoryType {
    ...
    /// Convert from string (database retrieval)
...
impl EpisodicMemory {
    ...
    pub fn conversation_turn(session_id: &str, turn_number: i32, content: &str) -> Self {
        Self::new(session_id, turn_number, content, MemoryType::ConversationTurn, None)
    }

    /// Create terminal output memory
    pub fn terminal_output(session_id: &str, turn_number: i32, output: &str) -> Self {
        Self::new(session_id, turn_number, output, MemoryType::TerminalOutput, None)
    }
    ...
    pub fn success_state(session_id: &str, turn_number: i32, message: &str) -> Self {
        Self::new(session_id, turn_number, message, MemoryType::SuccessState, Some(true))
    }
...
impl EpisodicStore {
    ...
    pub async fn log_action(
        ...
        let memory = EpisodicMemory::tool_execution(session_id, turn_number, action, output, success);
    ...
    pub async fn log_success(&self, session_id: &str, turn_number: i32, message: &str) -> Result<()> {
    ...
    pub async fn get_session_stats(&self, session_id: &str) -> Result<SessionStats> {
        ...
        let total: i64 =
            db.conn
                .query_row("SELECT COUNT(*) FROM episodic_memories WHERE session_id = ?", params![session_id], |r: &rusqlite::Row| r.get(0))?;
        ...
        let first_turn: Option<i32> = db
            .conn
            .query_row("SELECT MIN(turn_number) FROM episodic_memories WHERE session_id = ?", params![session_id], |r: &rusqlite::Row| {
                r.get(0)
            })
            .optional()?;
        ...
        let last_turn: Option<i32> = db
            .conn
            .query_row("SELECT MAX(turn_number) FROM episodic_memories WHERE session_id = ?", params![session_id], |r: &rusqlite::Row| {
                r.get(0)
            })
            .optional()?;
    ...
    pub async fn delete_session(&self, session_id: &str) -> Result<usize> {
        ...
        let rows = db
            .conn
            .execute("DELETE FROM episodic_memories WHERE session_id = ?", params![session_id])?;
...
mod tests {
    ...
    async fn test_retrieve_by_time_range() {
        ...
        let memories = store.retrieve_by_time_range("session-001", start, end).await.unwrap();
    ...
    async fn test_retrieve_trajectory() {
        ...
        // Log multiple turns
        store.log_conversation("session-001", 1, "Turn 1").await.unwrap();
        store.log_conversation("session-001", 2, "Turn 2").await.unwrap();
        store.log_conversation("session-001", 3, "Turn 3").await.unwrap();
    ...
    async fn test_session_isolation() {
        ...
        store.log_conversation("session-A", 1, "Session A").await.unwrap();
        store.log_conversation("session-B", 1, "Session B").await.unwrap();
        store.log_conversation("session-A", 2, "Session A again").await.unwrap();
    ...
    async fn test_turn_ordering() {
        ...
        // Log out of order
        store.log_conversation("session-001", 5, "Turn 5").await.unwrap();
        store.log_conversation("session-001", 2, "Turn 2").await.unwrap();
        store.log_conversation("session-001", 8, "Turn 8").await.unwrap();
        store.log_conversation("session-001", 1, "Turn 1").await.unwrap();
    ...
    async fn test_memory_type_filtering() {
        ...
        // Log different types
        store.log_conversation("session-001", 1, "Conversation").await.unwrap();
        store
            .log_action("session-001", 2, "cmd", "output", true)
            .await
            .unwrap();
        store.log_success("session-001", 3, "Success!").await.unwrap();
    ...
    async fn test_session_stats() {
        ...
        store.log_conversation("session-001", 1, "Start").await.unwrap();
        store
    ...
    async fn test_delete_session() {
        ...
        store.log_conversation("session-001", 1, "Test 1").await.unwrap();
        store.log_conversation("session-001", 2, "Test 2").await.unwrap();
        store.log_conversation("session-002", 1, "Other session").await.unwrap();
    ...
    async fn test_memory_type_conversion() {
        // Test to_string
        assert_eq!(MemoryType::ConversationTurn.to_string(), "conversation_turn");
        ...
        // Test from_str
        assert_eq!(
            MemoryType::from_str("conversation_turn").unwrap(),
            MemoryType::ConversationTurn
        );
        assert_eq!(
            MemoryType::from_str("TERMINAL_OUTPUT").unwrap(),
            MemoryType::TerminalOutput
        );
        assert!(MemoryType::from_str("invalid_type").is_err());
    }
    ...
    async fn test_session_stats_success_rate() {
        ...
        let stats = store.get_session_stats("session-all-success").await.unwrap();

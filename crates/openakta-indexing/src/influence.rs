...
//!
//! ```rust,no_run
//! use openakta_indexing::scip::{SCIPIndex, ParserRegistry, Language};
...
use crate::scip::{SCIPIndex, SymbolKind};
...
impl InfluenceGraph {
    ...
    fn count_symbols_in_file(&self, file_id: &str, _kind: SymbolKind) -> usize {
        self.file_to_symbols
            .get(file_id)
    ...
    pub fn calculate_transitive_closure(&self, file_id: &str) -> Result<Vec<FileId>> {
...
impl InfluenceGraph {
    ...
    pub fn add_edge(&mut self, from: &str, to: &str) -> Result<()> {
        ...
        self.dependencies.entry(from.to_string()).or_default();
        self.dependencies.entry(to.to_string()).or_default();
        self.reverse_dependencies
            .entry(from.to_string())
            .or_default();
        self.reverse_dependencies.entry(to.to_string()).or_default();

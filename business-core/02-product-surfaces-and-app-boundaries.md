...
Describe the actual runtime surfaces, interfaces, and package boundaries that make up OPENAKTA today.
...
- The daemon binary (`openakta-daemon`) is the main operational entry point for the backend.
- The gRPC server in `openakta-core` exposes the current network/API surface.
- `openakta-agents` contains the orchestration, task, workflow, provider, retrieval, and patch runtime.
- `openakta-cache` contains blackboard v2, TOON, prefix caching, diff parsing, and pruning utilities.
- `openakta-indexing` contains repository map, SCIP, influence graph, Merkle state, task queue, and indexing utilities.
- `openakta-storage` provides SQLite schema and partial persistence helpers.
...
- `crates/openakta-indexing/src/lib.rs`
- `apps/desktop/`
